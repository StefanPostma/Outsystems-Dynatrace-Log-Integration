...
Everything below was verified live against an internal Dynatrace test tenant — no OutSystems app runs there, so the RUM↔trace mechanism is proven against a different (PHP/Laravel) stack. The OutSystems-specific piece is a design, not yet verified end to end.
